...
# install local repeater
mkdir -p  ~/.sandbox-module/
cp -rf ../target/.sandbox-module/* ~/.sandbox-module/

# output zip file moobox-agent.zip
# monbox-agent.tar解压出来就是.sandbox-module（sandbox查找的自定义module的目录）, monbox-agent.tar和repeater-stable-bin.tar、repeater-module.jar是同样的内容
# 用于在业务服务器上拉起sandbox时加载moonbox-model使用(getRemoteAgentStartCommand)
cd ../target/.sandbox-module
# 去除Path contains '..'
tar -zcf  monbox-agent.tar -C ../ .

# cp monbox-agent.tar to local-agent
cp -f monbox-agent.tar ../${LOCAL_AGENT_DIR}/monbox-agent.tar